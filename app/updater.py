...
def set_record_ip(domain, record, ipaddr, token):
    ...
    if result['domain_record']['data'] == ipaddr:
        debug("Record {}.{} sucessfully updated to {}",
              record['name'], domain['name'], ipaddr)
    else:
        raise Exception(
            f"Could not set {record['name']}.{domain['name']} to {ipaddr}")
...
def process_args():
    parser = argparse.ArgumentParser(
        description='Updates DNS records in Digital Ocean')
...
def run(args):
    try:
        debug("Update {}.{} record type {}",
              args.record, args.domain, args.rtype)
